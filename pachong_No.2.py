...
def getDetailListPage(url):
    ...
    if not data2:
        if url[7] == 'l' or url[7] == 'L':
            ...
            return False
        elif url[7] == 's' or url[7] == 'S':
            print('search page')
            print('url: '+url)
            try:
                data2 = re.search(reStrSearch,data,re.S).group()
            except Exception as e:
                print(e)
                GdetailListPage.append(url)
                print('get search page number DONE')
                return True
        else:
...
def getData():
    ...
    reStrSearch = r'(?<=<div class="p-name">).*?(?=</a>)'
    ...
    for i in GdetailListPage:
        if i[7] == 'l' or i[7] == 'L':
            ...
            Snaker = snaker(reStrSearch,data)
            data2 = re.findall(reStrSearch,data,re.S)
            if not data2:
                print('Error: mistake occurs when analysising a detail search page')
                continue
...
def getSearchDetailName(data):
    reStr = r"(?<=>).*?(?=<font class='adwords')"
    reStr2 = r'(?<=title=").*?(?=")'
    try:
        temp_data = re.search(reStr,data,re.S).group()
    except:
        try:
            temp_data = re.search(reStr2,data,re.S).group()
        except Exception as e:
            print(data)
            print('error: when get search name')
            print(e)
            return None
    temp_data = temp_data.replace('<font class="skcolor_ljg">','')
    temp_data = temp_data.replace('</font>','')
...
def main():
    ...
    for i in range(len(listPage)):
        if getDetailListPage(listPage[i+6]):
            Gcategory = listCate[i+6]
            getData()
            Gcategory = None
            print(time.clock())
            gc.collect()
